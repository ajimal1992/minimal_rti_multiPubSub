...
class GoodbyeWorldListener : public DDSDataReaderListener {
  public:
    virtual void on_requested_deadline_missed(
        DDSDataReader* /*reader*/,
        const DDS_RequestedDeadlineMissedStatus& /*status*/) {}
...
void GoodbyeWorldListener::on_data_available(DDSDataReader* reader)
{
    GoodbyeWorldDataReader *GoodbyeWorld_reader = NULL;
    GoodbyeWorldSeq data_seq;
    DDS_SampleInfoSeq info_seq;
    DDS_ReturnCode_t retcode;
    int i;

    GoodbyeWorld_reader = GoodbyeWorldDataReader::narrow(reader);
    if (GoodbyeWorld_reader == NULL) {
        printf("DataReader narrow error\n");
        return;
    }

    retcode = GoodbyeWorld_reader->take(
        data_seq, info_seq, DDS_LENGTH_UNLIMITED,
        DDS_ANY_SAMPLE_STATE, DDS_ANY_VIEW_STATE, DDS_ANY_INSTANCE_STATE);
    ...
    for (i = 0; i < data_seq.length(); ++i) {
        if (info_seq[i].valid_data) {
            printf("Received data\n");
            GoodbyeWorldTypeSupport::print_data(&data_seq[i]);
        }
    }

    retcode = GoodbyeWorld_reader->return_loan(data_seq, info_seq);
    if (retcode != DDS_RETCODE_OK) {
        printf("return loan error %d\n", retcode);
    }
}
...
extern "C" int subscriber_main(int domainId, int sample_count)
{
    ...
    GoodbyeWorldListener *reader_listener2 = NULL; 
    ...
    /* Register the type before creating the topic */
    type_name2 = GoodbyeWorldTypeSupport::get_type_name();
    retcode = GoodbyeWorldTypeSupport::register_type(
    ...
    /* Create a data reader listener */
    reader_listener2 = new GoodbyeWorldListener();

    /* To customize the data reader QoS, use 
    the configuration file USER_QOS_PROFILES.xml */
    reader2 = subscriber2->create_datareader(
        topic2, DDS_DATAREADER_QOS_DEFAULT, reader_listener2,
        DDS_STATUS_MASK_ALL);
    if (reader2 == NULL) {
        printf("create_datareader error\n");
        subscriber_shutdown(participant);
